...
public class FirebaseClient {
    ...
    public static FirebaseClient shared() {
        if (instance == null) {
            synchronized (mutex) {
                if (instance == null) {
    ...
    private void loadConfig() {
        try {
            ...
            FileInputStream serviceAccount = new FileInputStream(config.getCredentialFile());

            FirebaseOptions options = new FirebaseOptions.Builder()
                    .setCredentials(GoogleCredentials.fromStream(serviceAccount))
                    .setDatabaseUrl(this.config.getDatabaseUrl())
                    .build();

            FirebaseApp.initializeApp(options);
            logger.info("#loadConfig - appname: %s +", FirebaseApp.getApps());
        } catch (Exception ex) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            if (false) {
                ...
                FirebaseClient.shared().send(token, null, "Test", "Test content here");
            }
            ...
            if (true) {
                ...
                FirebaseClient.shared().sendMultiple(logs, null, "Received money", "Received 100,000 HTG from abc xyz");
